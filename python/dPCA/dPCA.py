...
class dPCA(BaseEstimator):
    ...
    def _get_parameter_combinations(self, join=True):
        ...
        subsets = list(
            chain.from_iterable(
                combinations(range(len(self.labels)), r)
    ...
    def _marginalize(self, X, save_memory=False):
        ...
        # center data
        Xres -= np.mean(
            Xres.reshape((Xres.shape[0], -1)),
            -1).reshape((Xres.shape[0], ) + (len(Xres.shape) - 1) * (1, ))
        ...
        if save_memory:
            ...
            for key, phi in pcombs.items():
                key_without_phi = self.labels.replace(key, "")

                # build local dictionary for numexpr
                X = pre_mean[
                    key_without_phi] if len(key_without_phi) > 0 else Xres
    ...
    def _optimize_regularization(self, X, trialX, center=True, lams='auto'):
        ...
        # center data
        if center:
            X = X - np.mean(X.reshape(
                (X.shape[0],
                 -1)), 1).reshape((X.shape[0], ) + len(self.labels) * (1, ))
        ...
        totalscore = np.mean(
            np.sum(np.dstack([scores[key] for key in scores.keys()]), -1), 0)
        ...
        if np.argmin(totalscore) == 0 or np.argmin(
            ...
            if self.debug > 0:
                print(
                    'Warning: Optimal regularization parameter lies at the boundary of the search interval. Please provide different search list (key: lams).'
                )
    ...
    def crossval_score(self, lams, X, trialX, mean=True):
        ...
        for trial in range(self.n_trials):
            ...
            trainX, validX = self.train_test_split(
                X, trialX, N_samples=N_samples)
    ...
    def _score(self, X, mXs, mean=True):
        ...
        for key in mXs.keys():
            error[key] = np.sum((mXs[key] - PDY[key] + trPD[key][:, None] * X)
                                **2)
            ...
            pregY = np.dot(
                ...
                np.linalg.inv(
                    np.dot(
                        ...
                        Y.reshape((n_features,
                                   -1)).T) + lam**2 * np.eye(n_features)))
    ...
    def _zero_mean(self, X):
        ...
        return X - np.mean(X.reshape(
            (X.shape[0],
             -1)), 1).reshape((X.shape[0], ) + (len(X.shape) - 1) * (1, ))
    ...
    def _get_n_samples(self, trialX, protect=None):
        ...
        n_unprotect = len(
            trialX.shape) - len(protect) - 1 if protect is not None else len(
                trialX.shape) - 1
    ...
    def train_test_split(self, X, trialX, N_samples=None, sample_ax=0):
        ...
        # compute trainX
        trainX = (
            X * (N_samples / (N_samples - 1))[(np.s_[:], ) * n_unprotect +
                                              (None, ) * n_protect] - blindX /
            (N_samples - 1)[(np.s_[:], ) * n_unprotect + (None, ) * n_protect])

        # inverse rolled axis in blindX
        if ~protected:
            blindX = self._roll_back(
                blindX[..., None], axes, invert=True)[..., 0]
            trainX = self._roll_back(
                trainX[..., None], axes, invert=True)[..., 0]
    ...
    def transform(self, X, marginalization=None):
        ...
        if marginalization is not None:
            X_transformed = np.dot(
                self.D[marginalization].T, X.reshape(
                    (X.shape[0], -1))).reshape(
                        (self.D[marginalization].shape[1], ) + X.shape[1:])
        else:
            X_transformed = {}
            for key in self.marginalizations.keys():
    ...
    def inverse_transform(self, X, marginalization):
        ...
        X = self._zero_mean(X)
        X_transformed = np.dot(
            self.P[marginalization], X.reshape((X.shape[0], -1))).reshape(
